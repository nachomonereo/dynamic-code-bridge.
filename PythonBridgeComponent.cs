...
".Replace("{shortId}", shortId);
        ...
        protected override void AppendAdditionalComponentMenuItems(ToolStripDropDown menu)
        {
            ...
            Menu_AppendItem(menu, "Export as Standalone User Object", (s, e) => {
                ExportToUserObject();
            });
        ...
        private void ExportToUserObject()
        {
            try
            {
                Guid pythonGuid = new Guid("d60502dd-debd-400e-9c4c-3ba0a10eb537");
                foreach(var proxy in Grasshopper.Instances.ComponentServer.ObjectProxies) {
                    if (proxy.Desc.Name == "Python 3 Script" || proxy.Desc.Name == "Python 3") {
                        pythonGuid = proxy.Guid;
                        break;
                    }
                }

                var nativeComp = Grasshopper.Instances.ComponentServer.EmitObject(pythonGuid) as IGH_Component;
                if (nativeComp == null) {
                    throw new Exception("Could not instantiate the native Grasshopper Python component.");
                }

                var vpc = nativeComp as IGH_VariableParameterComponent;
                if (vpc != null) {
                    nativeComp.Params.Input.Clear();
                    nativeComp.Params.Output.Clear();

                    foreach (var p in this.Params.Input) {
                        var newParam = vpc.CreateParameter(GH_ParameterSide.Input, nativeComp.Params.Input.Count);
                        newParam.Name = p.Name;
                        newParam.NickName = p.NickName;
                        newParam.Description = p.Description;
                        newParam.Access = p.Access;
                        nativeComp.Params.RegisterInputParam(newParam);
                    }

                    var codeOutParam = vpc.CreateParameter(GH_ParameterSide.Output, nativeComp.Params.Output.Count);
                    codeOutParam.Name = "CodeOut";
                    codeOutParam.NickName = "C";
                    codeOutParam.Description = "Source code of this component for Code Bridge";
                    nativeComp.Params.RegisterOutputParam(codeOutParam);

                    foreach (var p in this.Params.Output) {
                        if (p.Name == "Diagnostics") continue;
                        var newParam = vpc.CreateParameter(GH_ParameterSide.Output, nativeComp.Params.Output.Count);
                        newParam.Name = p.Name;
                        newParam.NickName = p.NickName;
                        newParam.Description = p.Description;
                        nativeComp.Params.RegisterOutputParam(newParam);
                    }

                    vpc.VariableParameterMaintenance();
                }

                string codeToInject = _lastCode + "\r\n\r\nCodeOut = \"\"\"" + _lastCode.Replace("\\", "\\\\").Replace("\"", "\\\"") + "\"\"\"";
                
                dynamic dynComp = nativeComp;
                try {
                    dynComp.SetSource(codeToInject);
                } catch {
                    try { dynComp.Code = codeToInject; } catch { }
                }

                var userObj = new Grasshopper.Kernel.GH_UserObject();
                userObj.SetDataFromObject(nativeComp);
                userObj.Description.Name = this.Name + " (Standalone)";
                userObj.Description.Description = "Standalone Python script generated by Dynamic Code Bridge.\nCredits: Robots Lab and IAAC. Author: Nacho Monereo.";
                userObj.Description.Category = "IAAC";
                userObj.Description.SubCategory = "PY Script";
                userObj.Icon = this.Icon_24x24;

                var formType = typeof(Grasshopper.Instances).Assembly.GetType("Grasshopper.GUI.GH_UserObjectPropertiesForm");
                if (formType != null) {
                    var form = Activator.CreateInstance(formType, new object[] { userObj }) as System.Windows.Forms.Form;
                    if (form != null) {
                        var result = form.ShowDialog(Grasshopper.Instances.DocumentEditor);
            ...
            {
                System.Windows.Forms.MessageBox.Show("Failed to export User Object: " + ex.Message, "Export Error", System.Windows.Forms.MessageBoxButtons.OK, System.Windows.Forms.MessageBoxIcon.Error);
            }
        }
    }
